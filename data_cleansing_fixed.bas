Attribute VB_Name = "modToolsDataCleansing"
' ------------------------------------------------------
...
' Author: Corey Brosam
' Date: 01/12/2021
' Modified: 2025-11-07 - Phase 1 Critical Bug Fixes Applied
'   - Fixed Too_Many_Cells logic error (line 699 duplicate assignment)
'   - Fixed Change_Case sentence case bug on filtered ranges (line 280)
'   - Fixed Application.Calculation restoration in error handlers (6 procedures)
' ------------------------------------------------------
Option Explicit
...
Sub Blank_Cells()
    ...
    Dim originalCalcMode As XlCalculation         ' PHASE1 FIX: Store original state
    Dim originalScreenUpdating As Boolean         ' PHASE1 FIX: Store original state
    ...
    ' PHASE1 FIX: Store original state before changing
    originalCalcMode = Application.Calculation
    originalScreenUpdating = Application.ScreenUpdating
    
    Application.Calculation = xlCalculationManual
    Application.ScreenUpdating = False            ' PHASE1 FIX: Added for performance
...
TidyUp:
    ...
    ' PHASE1 FIX: Always restore original state, even on error
...
Sub Change_Case(id As String)
    ...
    ' PHASE1 FIX: Removed unused MyString variable declaration
    Dim originalCalcMode As XlCalculation         ' PHASE1 FIX: Store original state
    Dim originalScreenUpdating As Boolean         ' PHASE1 FIX: Store original state
        ...
        For Each cell In rng
            ...
            If cell.EntireRow.Hidden = False Then
                ...
                Else
                    ...
                    ' PHASE1 FIX: Apply ProperCaps result to cell (was: MyString = ProperCaps(...))
...
Public Function Too_Many_Cells(lrow As Long, lcol As Long) As Boolean
    '
    ' PHASE1 FIX: Completely rewritten to fix logic error
    '
    ...
    '
    ' CHANGES:
    '   - Fixed duplicate assignment bug (old line 699)
    '   - Fixed error handling for overflow (check after calculation)
    '   - Added formatted cell count in user message
